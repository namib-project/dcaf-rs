...
use coset::{
    iana, AsCborValue, CborSerializable, CoseError, CoseKey, CoseKeyBuilder, CoseSign1,
    CoseSign1Builder, Header, HeaderBuilder, Label, TaggedCborSerializable,
};
use hex::FromHex;
...
fn verify_sign1_test_case<T: CoseSignCipher>(
    ...
    let verify_result = sign1.try_verify(backend, &Some(&key), false, aad);
...
fn perform_reference_output_test(test_path: PathBuf, mut backend: impl CoseSignCipher) {
...
fn perform_self_signed_test(test_path: PathBuf, mut backend: impl CoseSignCipher) {
    let test_case_description: TestCase =
        serde_json::from_reader(std::fs::File::open(test_path).expect("unable to open test case"))
            .expect("invalid test case");
...
fn cose_examples_ecdsa_sign1_reference_output(
    ...
    perform_reference_output_test(test_path, backend)
}
...
fn cose_examples_ecdsa_sign1_self_signed(
    ...
    perform_self_signed_test(test_path, backend)
}
